...
def load_modules(refresh: bool = False):
    ...
    search_paths = [
        script_path / "starter_files" / "utils" / "oss" / os_name / os_version,  # 1. Самая специфичная
        script_path / "starter_files" / "utils" / "oss" / os_name / "default",    # 2. Общая для ОС
        script_path / "starter_files" / "utils" / "oss" / "default",              # 3. Общая реализация
    ]
...
def _get_oss_tree() -> List[Tuple[Path, str, Optional[str], int]]:
    ...
    """
    script_path = Path(get_global('script_path'))
    base_dir = script_path / 'starter_files' / 'utils' / 'oss'
    items = []
