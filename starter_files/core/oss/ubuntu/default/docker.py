...
class DockerModule(BaseModule):
    ...
    def return_commands_install() -> List[str]:
        """Возвращает список команд для установки Docker в Ubuntu"""
        ...
        commands = [
            ...
            f"{prefix}curl -fsSL https://download.docker.com/linux/ubuntu/gpg | {prefix}gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg",
            f"{prefix}echo \"deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable\" | {prefix}tee /etc/apt/sources.list.d/docker.list > /dev/null",
            f"{prefix}apt update",
