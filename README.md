<H1>Стартер | 启动器 | Starter</H1>
<hr />

С инструкцией вы можете ознакомиться тут <a href="https://myidon.site/instruction/starter">Инструкция</a>
<hr />
您可以在 <a href="https://myidon.site/instruction/starter">这里</a> 查看说明。
<hr />
You can check the instructions <a href="https://myidon.site/instruction/starter">here</a>.

<div style="color: red; font-size: large; text-align: center;">
ЕСЛИ ВЫ НЕ ЯВЛЯЕТЕСЬ АДМИНИСТРАТОРОМ ИЛИ СОТРУДНИКОМ ПРОЕКТА — НЕ УСТАНАВЛИВАЙТЕ ЭТУ ПРОГРАММУ!<br/>
УСТАНОВКА ЭТОЙ ПРОГРАММЫ ПРИВЕДЕТ К ПОТЕРЕ КОНТРОЛЯ НАД ВАШИМ УСТРОЙСТВОМ.<br/>
УДАЛИТЬ ЕЁ МОЖНО ЛИШЬ ФИЗИЧЕСКИМ ДОСТУПОМ!<br/><br/>
如果你不是项目的管理员或员工，请不要安装此程序！<br/>
安装该程序将导致设备控制权丢失。<br/>
只有通过物理访问才能删除它！<br/><br/>
IF YOU ARE NOT AN ADMINISTRATOR OR EMPLOYEE OF THE PROJECT DO NOT INSTALL THIS PROGRAM!<br/>
INSTALLING THIS PROGRAM WILL RESULT IN LOSS OF CONTROL OVER YOUR DEVICE.<br/>
IT CAN ONLY BE REMOVED BY PHYSICAL ACCESS TO IT!
</div>