...
logger = LogManager.get_logger('fs')
...
class FsModule(BaseModule):
    ...
    def set_globals():
        ...
        # Здесь можно установить пути для файловой системы, если нужно
        starter_path = get_global('starter_path')
        if starter_path:
            # Пример: установка директории для бэкапов
            backup_dir = starter_path / "files" / "backups"
            set_global('fs_backup_dir', backup_dir)
            logger.debug(f"FS backup directory: {backup_dir}")
        ...
        for pattern in patterns:
            for src_path in source.glob(pattern):
                if src_path.is_file():
                    ...
                    elif FsModule.get_file_hash(str(src_path)) != FsModule.get_file_hash(str(dst_path)):
