...
class GitModule(BaseModule):
    """Git утилиты для Windows — скрытие окон"""
    ...
    def check_git_authentication() -> str:
        ...
        if not GitModule.check_git_installed():
    ...
    def authenticate_git(username: str, password: str) -> bool:
        ...
        try:
            ...
            subprocess.run(['git', 'config', '--global', 'user.name', username], check=True, startupinfo=_si())
            subprocess.run(['git', 'config', '--global', 'user.email', f"{username}@users.noreply.github.com"], check=True, startupinfo=_si())
