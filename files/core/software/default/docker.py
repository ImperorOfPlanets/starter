...
class DockerModule(BaseModule):
    ...
    def run_compose(log_path: Path = None) -> bool:
        ...
        try:
            ...
            # ИСПРАВЛЕНИЕ: меняем check=True на check=False
            try:
                down_process = subprocess.run(
                    ...
                    capture_output=True,
                    text=True,
                    check=False  # ВАЖНО: check=False вместо check=True
                )
                ...
                # Продолжаем выполнение, не падаем
            ...
            time.sleep(2)  # Даем время контейнерам на запуск
