...
class DockerModule(BaseModule):
    ...
    def ensure_docker_env(log_path: Optional[Path] = None) -> Dict[str, str]:
        ...
        # ИСПРАВЛЕНО: используем docker_path из глобальных переменных
        docker_path = get_global('docker_path')
        if not docker_path:
            log("   ❌ docker_path не установлен в глобальных переменных!")
            if log_path:
                with open(log_path, 'a', encoding='utf-8') as log_file:
        ...
        log("")
        log(f"   📂 ensure_docker_env()")
        log(f"      📄 docker_env_example_path: {docker_env_example_path}")
        log(f"      📄 docker_env_path: {docker_env_path}")
        ...
        if not docker_env_example_path.exists():
            log(f"      ⚠️  .env.example НЕ НАЙДЕН: {docker_env_example_path}")
            if log_path:
                with open(log_path, 'a', encoding='utf-8') as log_file:
                    log_file.write("[ensure_docker_env] ERROR: .env.example not found\n")
            return {}
        else:
            log(f"      ✅ .env.example найден")
        ...
        try:
            ...
            log(f"      📋 Переменных в .env.example: {len(example_vars)}")
            ...
            log(f"      ❌ Ошибка парсинга .env.example: {e}")
            if log_path:
                with open(log_path, 'a', encoding='utf-8') as log_file:
        ...
        if docker_env_path.exists():
            log(f"      ✅ .env найден, читаем текущие переменные...")
            if log_path:
                with open(log_path, 'a', encoding='utf-8') as log_file:
            ...
            try:
                ...
                log(f"      📋 Текущих переменных в .env: {len(current_vars)}")
        ...
        try:
            ...
            log(f"      💾 Запись в: {docker_env_path}")
            ...
            log(f"      ✅ .env записан ({len(content)} символов)")
    ...
    def read_docker_env() -> Dict[str, str]:
        """
        Читает docker/.env и возвращает словарь переменных.
        """
        # ИСПРАВЛЕНО: используем docker_path из глобальных переменных
        docker_path = get_global('docker_path')
        if not docker_path:
        ...
        if not docker_env_path.exists():
            log(f"      ⚠️  Файл НЕ НАЙДЕН: {docker_env_path}")
            return {}

        log(f"      📖 Чтение: {docker_env_path}")
        try:
            with open(docker_env_path, 'r', encoding='utf-8') as f:
                content = f.read()
            vars_dict, _ = DockerModule.parse_env_content(content)
            log(f"      📋 Прочитано переменных: {len(vars_dict)}")
            return vars_dict
        except Exception as e:
    ...
    def process_port_plus_variables(base_port: int):
        """
        Опционально обрабатывает PORT_*_PLUS переменные из ./docker/.env.example
        Если переменных нет - просто пропускает
        """
        docker_path = get_global('docker_path')
        if not docker_path:
            print("   ❌ docker_path не установлен в глобальных переменных!")
            return
        ...
        if not env_example_path.exists():
            print(f"   ℹ️ Файл .env.example не найден, пропускаем обработку PORT_*_PLUS")
            return
            ...
            if '=' in stripped:
                ...
                if key.endswith('_PLUS'):
                    try:
                        ...
                        plus_count += 1
                        print(f"         📊 {key}={plus_value} → {base_name}={computed_port}")
                    except ValueError:
                        print(f"         ⚠️ {key}={raw_val} — некорректное значение, пропускаем")
                        continue
        
        # Если есть PORT_*_PLUS - обновляем .env
        if plus_count > 0:
            # Читаем текущий .env
            current_vars = {}
            ...
            print(f"      ✅ Добавлено {plus_count} PORT_* переменных (базовый порт {base_port})")
        else:
            print(f"      ℹ️ Нет PORT_*_PLUS переменных в .env.example, пропускаем")

    @staticmethod
    def allocate_network_and_ports() -> Dict[str, Any]:
        """
        Полностью автономная функция для выделения сети и портов.
        """
        import traceback
        
        def log(msg: str, level: str = "INFO"):
            """Выводит сообщение с уровнем"""
            prefix = {
                "INFO": "ℹ️",
                "SUCCESS": "✅",
                "WARNING": "⚠️",
        ...
        log("=" * 80)
        log("🌐 НАЧАЛО ВЫДЕЛЕНИЯ СЕТИ И ПОРТОВ", "INFO")
        log("=" * 80)

        # ========== ДИАГНОСТИКА: что у нас есть ==========
        log("", "INFO")
        log("📋 ДИАГНОСТИКА ИСХОДНЫХ ДАННЫХ", "INFO")
        log("-" * 60, "INFO")
        ...
        log(f".env.example существует: {env_example.exists() if env_example else 'N/A'}", "DEBUG")
        log(f".env существует: {env_file.exists() if env_file else 'N/A'}", "DEBUG")
        
        # ========== ШАГ 1: Реестр проектов ==========
        log("", "INFO")
        log("📋 ШАГ 1: ПОЛУЧЕНИЕ ЗАНЯТЫХ ОКТЕТОВ ИЗ РЕЕСТРА", "INFO")
        log("-" * 60, "INFO")
        ...
        if registry_module is None:
            log("RegistryModule не найден через get('registry')!", "WARNING")
        else:
            log(f"RegistryModule найден: {registry_module}", "SUCCESS")
            try:
                exclude_path = str(project_path) if project_path else ""
                log(f"Вызов get_used_octets(exclude_path='{exclude_path}')", "DEBUG")
                used_octets_registry = set(registry_module.get_used_octets(exclude_path))
                log(f"Занятые октеты из реестра: {sorted(used_octets_registry)}", "INFO")
            except Exception as e:
                log(f"Ошибка при получении октетов из реестра: {e}", "ERROR")
                log(traceback.format_exc(), "DEBUG")
                used_octets_registry = set()
        
        # ========== ШАГ 2: Docker сети ==========
        log("", "INFO")
        log("📋 ШАГ 2: ПОЛУЧЕНИЕ ЗАНЯТЫХ ПОДСЕТЕЙ ИЗ DOCKER", "INFO")
        log("-" * 60, "INFO")
        ...
        try:
            ...
            if docker_version.returncode == 0:
                log(f"Docker доступен: {docker_version.stdout.strip()}", "SUCCESS")
            else:
                log("Docker не отвечает!", "WARNING")
        except Exception as e:
            log(f"Не удалось проверить Docker: {e}", "WARNING")
        
        try:
            used_subnets = DockerModule.get_used_subnets()
            log(f"Найдено подсетей Docker: {len(used_subnets)}", "INFO")
            ...
            for subnet in used_subnets:
                ...
                try:
                    ...
                    if len(parts) >= 2:
                        ...
                        log(f"    → октет {octet_val}", "DEBUG")
                except (IndexError, ValueError) as e:
                    log(f"    → ошибка извлечения октета: {e}", "WARNING")
                    
            log(f"Занятые октеты в Docker: {sorted(used_octets_docker)}", "INFO")
        except Exception as e:
            log(f"Ошибка при получении подсетей Docker: {e}", "ERROR")
            log(traceback.format_exc(), "DEBUG")
        
        # ========== ШАГ 3: Поиск свободного октета ==========
        log("", "INFO")
        log("📋 ШАГ 3: ПОИСК СВОБОДНОГО ОКТЕТА", "INFO")
        log("-" * 60, "INFO")
        
        preferred_octet = 20
        max_octet = 65
        
        log(f"Диапазон поиска: {preferred_octet} - {max_octet}", "INFO")
        log(f"Исключаем октеты из реестра: {sorted(used_octets_registry)}", "DEBUG")
        log(f"Исключаем октеты из Docker: {sorted(used_octets_docker)}", "DEBUG")
        ...
        for octet in range(preferred_octet, max_octet + 1):
            attempts += 1
            log("", "DEBUG")
            log(f"🔄 ПРОВЕРКА ОКТЕТА {octet} (попытка {attempts})", "INFO")
            log(f"   {'─' * 50}", "DEBUG")
            ...
            if octet in used_octets_registry:
                reason = f"октет {octet} занят в реестре"
                log(f"   ❌ {reason}", "WARNING")
                skipped_reasons[octet] = reason
                continue
            else:
                log(f"   ✅ октет {octet} свободен в реестре", "DEBUG")
            
            # Проверка в Docker
            if octet in used_octets_docker:
                reason = f"подсеть 172.{octet}.0.0/16 занята в Docker"
                log(f"   ❌ {reason}", "WARNING")
                skipped_reasons[octet] = reason
                continue
            else:
                log(f"   ✅ подсеть 172.{octet}.0.0/16 свободна в Docker", "DEBUG")
            
            # Вычисляем базовый порт
            base_port = octet * 100
            log(f"   📐 базовый порт = {octet} × 100 = {base_port}", "DEBUG")
            
            # Проверка базового порта
            port_manager = get('portmanager')
            if port_manager is None:
                log(f"   ❌ PortmanagerModule не найден!", "ERROR")
                reason = "PortmanagerModule не найден"
                skipped_reasons[octet] = reason
                continue
            ...
            try:
                ...
                if not is_free:
                    reason = f"базовый порт {base_port} занят"
                    log(f"   ❌ {reason}", "WARNING")
                    skipped_reasons[octet] = reason
                    continue
                else:
                    log(f"   ✅ базовый порт {base_port} свободен", "DEBUG")
            except Exception as e:
                log(f"   ❌ ошибка проверки порта {base_port}: {e}", "ERROR")
                reason = f"ошибка проверки порта: {e}"
                skipped_reasons[octet] = reason
                continue
            
            # Генерация docker/.env
            log(f"   📝 генерация временного docker/.env...", "DEBUG")
            try:
                get('docker', 'ensure_docker_env')
                get('docker', 'process_port_plus_variables', base_port)
                log(f"   ✅ временный .env сгенерирован", "DEBUG")
            except Exception as e:
                log(f"   ❌ ошибка генерации .env: {e}", "ERROR")
                reason = f"ошибка генерации .env: {e}"
                skipped_reasons[octet] = reason
                continue
            
            # Чтение портов из docker/.env
            log(f"   📖 чтение PORT_* переменных из docker/.env...", "DEBUG")
            try:
                ...
                if not docker_vars:
                    log(f"   ⚠️ docker_vars пуст, но это не критично", "WARNING")
                
                # Собираем PORT_* переменные (если они есть)
                ports_to_check = []
                has_port_vars = False
                ...
                for key, val in docker_vars.items():
                    if key.startswith('PORT_'):
                        ...
                        try:
                            ...
                            log(f"     📊 {key} = {val} → порт {port_num}", "DEBUG")
                        except ValueError:
                            log(f"     ⚠️ {key} = {val} — не число, пропускаем", "WARNING")
                
                # Если нет PORT_* переменных, проверяем только базовый порт
                if not has_port_vars:
                    log(f"   ℹ️ Нет PORT_* переменных в docker/.env, проверяем только базовый порт {base_port}", "INFO")
                    ports_to_check = [base_port]
                    log(f"   📋 портов для проверки: {ports_to_check}", "INFO")
                else:
                    log(f"   📋 найдено PORT_* переменных: {len(ports_to_check)}", "INFO")
                    log(f"   📋 список портов: {ports_to_check}", "DEBUG")
                
                # Проверка каждого порта
                log(f"   🔍 проверка портов на занятость...", "DEBUG")
                all_free = True
                busy_ports = []
                ...
                for idx, p in enumerate(ports_to_check, 1):
                    try:
                        ...
                        if not is_free:
                            log(f"     [{idx}/{len(ports_to_check)}] порт {p} ❌ ЗАНЯТ", "WARNING")
                            all_free = False
                            busy_ports.append(p)
                        else:
                            log(f"     [{idx}/{len(ports_to_check)}] порт {p} ✅ СВОБОДЕН", "DEBUG")
                    except Exception as e:
                        log(f"     [{idx}/{len(ports_to_check)}] порт {p} ❌ ОШИБКА: {e}", "ERROR")
                        all_free = False
                        busy_ports.append(p)
                
                if not all_free:
                    reason = f"порты заняты: {busy_ports}"
                    log(f"   ❌ {reason}", "WARNING")
                    skipped_reasons[octet] = reason
                    continue
                
                # ВСЕ ПРОВЕРКИ ПРОЙДЕНЫ!
                log(f"   🎉 ВСЕ ПРОВЕРКИ ПРОЙДЕНЫ!", "SUCCESS")
                final_octet = octet
                ...
                log(f"   ❌ ошибка при обработке октета {octet}: {e}", "ERROR")
                log(traceback.format_exc(), "DEBUG")
                reason = f"ошибка: {e}"
                skipped_reasons[octet] = reason
                continue
        ...
        if final_octet is None:
            log("❌ НЕ УДАЛОСЬ НАЙТИ СВОБОДНУЮ КОМБИНАЦИЮ!", "ERROR")
            log(f"", "INFO")
            log(f"📊 СТАТИСТИКА:", "INFO")
            log(f"   🔄 Всего попыток: {attempts}", "INFO")
            log(f"   📋 Занятые октеты (реестр): {sorted(used_octets_registry)}", "INFO")
            log(f"   🐳 Занятые октеты (Docker): {sorted(used_octets_docker)}", "INFO")
            log(f"", "INFO")
            log(f"📋 ПРИЧИНЫ ПРОПУСКА ОКТЕТОВ:", "INFO")
            ...
            raise RuntimeError(f"Не найдено свободного октета в диапазоне {preferred_octet}–{max_octet}. "
                            f"Реестр: {sorted(used_octets_registry)}, Docker: {sorted(used_octets_docker)}")
        
        # ========== СОХРАНЕНИЕ РЕЗУЛЬТАТОВ ==========
        log("🎉 ВЫДЕЛЕНИЕ СЕТИ И ПОРТОВ ЗАВЕРШЕНО УСПЕШНО!", "SUCCESS")
        log(f"   📌 Октет: {final_octet}", "SUCCESS")
        log(f"   📌 Базовый порт: {final_base_port}", "SUCCESS")
        log(f"   📌 Подсеть: 172.{final_octet}.0.0/16", "SUCCESS")
        log(f"   📌 Порты: {final_ports}", "SUCCESS")
        log("=" * 80, "INFO")
